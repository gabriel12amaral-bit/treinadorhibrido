...
export const Route = createRootRouteWithContext<{ queryClient: QueryClient }>()({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Musculação, corrida e esportes em um único plano personalizado por IA.",
      },
      ...
      },
      {
        property: "og:image",
        content:
          "https://pub-bb2e103a32db4e198524a2e9ed8f35b4.r2.dev/834341c0-fffe-461e-9006-0faf36402c72/id-preview-347c8d23--22b6fcb3-f4c7-41d6-8fd9-96aa8bbb9120.lovable.app-1781754717452.png",
      },
      {
        name: "twitter:image",
        content:
...
function AppFrame() {
  ...
  return (
    <div className="mx-auto min-h-[100dvh] max-w-[430px] overflow-x-hidden bg-background text-foreground">
      <div
        style={{ paddingBottom: hideNav ? undefined : "calc(80px + env(safe-area-inset-bottom))" }}
      >
        <Outlet />
      </div>
      {!hideNav && <BottomNav />}
          ...
          <li key={to} className="flex-1">
            ...
            >
              {({ isActive }) => (
                <>
                  <Icon
                    className={`size-5 transition ${isActive ? "text-primary" : "text-muted-foreground"}`}
                    strokeWidth={isActive ? 2.5 : 1.75}
                  />
                  <span
                    className={`font-mono text-[9px] uppercase tracking-widest ${isActive ? "text-primary" : "text-muted-foreground/70"}`}
                  >
                    {label}
                  </span>
                  <span
                    className={`h-0.5 w-6 rounded-full transition ${isActive ? "bg-primary" : "bg-transparent"}`}
                  />
                </>
              )}
            </Link>
