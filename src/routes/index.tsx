  ...
  return (
    <div className="pb-32">
      <header className="px-5 pt-10 pb-5 animate-reveal">
        ...
        <div className="mt-1 flex items-end justify-between gap-4">
          <h1 className="font-display text-4xl uppercase leading-none tracking-tight">
            {day.rest ? "Recuperação" : day.title}
          </h1>
          <StreakChip value={streak} />
        </div>
        <p className="mt-2 text-sm text-muted-foreground">
          Olá, <span className="text-foreground">{profile?.name}</span> — {day.focus}.
        </p>
      </header>
      ...
      {recovery && (
        <Link
          to="/recuperacao"
          className="mx-5 mb-4 flex items-center justify-between rounded-xl border border-border bg-surface p-3 animate-reveal [animation-delay:40ms]"
        >
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="font-display text-base uppercase">
                {recovery.readiness} • Fadiga {recovery.fatigue}
              </p>
            </div>
          </div>
          <div className="flex gap-3 font-mono text-[10px]">
            <div className="text-right">
              <p className="text-muted-foreground">MUS</p>
              <p className="text-primary">{recovery.muscular}%</p>
            </div>
            <div className="text-right">
              <p className="text-muted-foreground">CAR</p>
              <p className="text-cardio">{recovery.cardio}%</p>
            </div>
          </div>
        </Link>
      )}
      ...
      <div className="mx-5 mb-4 grid grid-cols-2 gap-2 animate-reveal [animation-delay:45ms]">
        <Link
          to="/desempenho"
          className="rounded-xl border border-primary/30 bg-gradient-to-br from-primary/10 to-transparent p-3"
        >
          ...
          <p className="mt-1 font-display text-2xl leading-none">
            {ip?.score ?? "—"}
            <span className="ml-1 text-sm text-primary">{ip?.grade ?? ""}</span>
          </p>
          <p className="font-mono text-[9px] uppercase text-muted-foreground">{ip?.trend ?? ""}</p>
        </Link>
        <Link
          to="/objetivos"
          className={`rounded-xl border p-3 ${nextEvent ? "border-warning/40 bg-warning/5" : "border-border bg-surface"}`}
        >
          <div className="flex items-center gap-2">
          ...
          {nextEvent ? (
            <>
              <p className="mt-1 font-display text-2xl leading-none text-warning">
                D-{daysUntil(nextEvent.date)}
              </p>
      ...
      <Link
        to="/insights"
        className="mx-5 mb-4 flex items-center justify-between rounded-xl border border-primary/30 bg-gradient-to-r from-primary/10 to-transparent p-3 animate-reveal [animation-delay:50ms]"
      >
        <div className="flex items-center gap-3">
        ...
        <main className="px-5 space-y-6">
          <section className="grid grid-cols-3 gap-px overflow-hidden rounded-xl bg-border animate-reveal [animation-delay:80ms]">
            ...
            <Stat
              icon={Flame}
              label="Status"
              value={completed ? "Feito" : "Pronto"}
              accent={completed ? "primary" : undefined}
            />
          </section>
            ...
            <section className="space-y-3 animate-reveal [animation-delay:160ms]">
              <div className="flex items-center justify-between">
                ...
                <span className="font-mono text-xs text-muted-foreground">
                  {day.strength.length} exercícios
                </span>
              </div>
                ...
                {day.strength.map((s, i) => {
                  ...
                  return (
                    <li key={s.exerciseId}>
                      ...
                      >
                        <div className="flex items-center gap-3">
                          <div className="grid size-12 place-items-center rounded-lg bg-black/40 text-2xl ring-1 ring-white/5">
                            {image}
                          </div>
                          <div>
                            <p className="text-sm font-bold uppercase">{name}</p>
                            <p className="font-mono text-[11px] text-muted-foreground">
                              {s.sets} séries • {s.reps} reps • {s.rest}
                            </p>
                          </div>
                        </div>
                        <span className="rounded bg-primary/10 px-2 py-1 font-mono text-[10px] font-bold uppercase text-primary ring-1 ring-primary/20">
                          {group.slice(0, 5)}
                        </span>
                      </Link>
                    </li>
          ...
          {day.cardio && (
            <section className="rounded-r-xl border-l-4 border-cardio bg-surface p-4 animate-reveal [animation-delay:240ms]">
              <div className="mb-3 flex items-start justify-between">
                <div>
                  <h2 className="font-display text-xl uppercase tracking-wide text-cardio flex items-center gap-2">
                    <Footprints className="size-5" />
                    Corrida
                  </h2>
                  <p className="text-xs text-muted-foreground">{day.cardio.details}</p>
                </div>
                <span className="rounded bg-cardio/20 px-2 py-1 font-mono text-[10px] font-bold uppercase text-cardio">
                  {day.cardio.type.split(" ")[1] ?? day.cardio.type}
                </span>
              </div>
              <div className="grid grid-cols-3 gap-2">
                ...
                <MiniStat
                  label="Distância"
                  value={day.cardio.distance ? `${day.cardio.distance}km` : "—"}
                />
              </div>
            </section>
          )}
          ...
          {day.sport && (
            <section className="rounded-xl border border-warning/40 bg-warning/5 p-4 animate-reveal [animation-delay:300ms]">
              <h2 className="font-display text-lg uppercase tracking-wide text-warning">
                ⚽ {day.sport.sport}
              </h2>
              <p className="mt-1 text-xs text-muted-foreground">
                {day.sport.duration} min — {day.sport.details}
              </p>
            </section>
          )}
          ...
          <div className="grid grid-cols-[1fr_auto] gap-2">
            <button
              onClick={() =>
                navigate({
                  to: "/exercicio/$id",
                  params: { id: day.strength[0]?.exerciseId ?? "supino-reto" },
            ...
            >
              {completed ? (
                ...
                <>
                  <Play className="size-5 fill-background" /> Iniciar
                </>
              )}
            </button>
            <Link
              to="/atividade"
              className="grid place-items-center rounded-xl border border-border bg-surface px-4 active:scale-[0.98]"
...
function Stat({
  ...
  return (
    <div className="bg-surface p-3">
      ...
      <p className={`font-mono text-base font-bold ${accent === "primary" ? "text-primary" : ""}`}>
        {value}
      </p>
    </div>
  );
  ...
  return (
    <main className="px-5">
      <div className="rounded-xl border border-border bg-surface p-6 text-center animate-reveal">
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          A recuperação faz parte do plano. Hidrate-se, durma 8h e volte mais forte.
        </p>
      </div>
    </main>
  );
