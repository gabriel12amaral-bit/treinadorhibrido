...
import {
  type Profile, type WeekdayKey, type SportPractice, type SportIntensity, type Restriction,
  WEEKDAY_KEYS, WEEKDAY_LABELS, WEEKDAY_SHORT, SPORT_OPTIONS, RESTRICTION_OPTIONS,
} from "@/lib/hybrid";
...
const EQUIPS = ["Barra", "Halteres", "Anilhas", "Banco", "Polia", "Máquinas", "Elásticos", "Kettlebell", "Nenhum"];
...
function Onboarding() {
  ...
  const [p, setP] = useState<Draft>({
    sex: "M", goal: "Performance Híbrida", strengthLevel: "Intermediário",
    runLevel: "Intermediário", timePerDay: 60, location: "Academia", equipment: [],
    gymDays: ["seg", "qua", "sex"], sports: [], restrictionsList: ["Nenhuma"],
  });
  ...
  const next = () => {
    if (step === total - 1) {
      const profile: Profile = {
        name: p.name!, sex: p.sex!, age: p.age!, height: p.height!, weight: p.weight!,
        bodyFat: p.bodyFat, goal: p.goal!, strengthLevel: p.strengthLevel!, runLevel: p.runLevel!,
        daysPerWeek: p.gymDays.length || 3,
        timePerDay: p.timePerDay!, location: p.location!, equipment: p.equipment!,
        gymDays: p.gymDays,
        sports: p.sports,
  ...
  return (
    <div className="flex min-h-screen flex-col px-5 pt-10 pb-8">
      <div className="mb-6 flex items-center gap-1">
        {steps.map((_, i) => (
          <span key={i} className={`h-1 flex-1 rounded-full ${i <= step ? "bg-primary" : "bg-white/10"}`} />
        ))}
      </div>
      ...
      <div className="mt-6 flex gap-3">
        {step > 0 && (
          <button onClick={back} className="flex items-center justify-center rounded-xl border border-border px-4 py-3 text-sm">
        ...
        >
          {step === total - 1 ? <>Gerar Plano <Sparkles className="size-4" /></> : <>Continuar <ChevronRight className="size-5" /></>}
        </button>
      </div>
    </div>
  );
...
function validate(key: string, p: Draft): boolean {
  switch (key) {
    ...
    case "basics": return !!p.name && !!p.age && !!p.sex;
    case "body": return !!p.height && !!p.weight;
    case "goal": return !!p.goal;
    case "level": return !!p.strengthLevel && !!p.runLevel;
    case "gymDays": return p.gymDays.length > 0;
    case "time": return !!p.timePerDay;
    case "place": return !!p.location && (p.equipment?.length ?? 0) > 0;
    case "sports": return true;
    case "sportsDays": return p.sports.every((s) => s.days.length > 0);
    case "restrictions": return p.restrictionsList.length > 0;
...
function Intro() {
  return (
    <div className="flex h-full flex-col justify-center">
      <p className="font-mono text-[10px] uppercase tracking-[0.3em] text-primary">Hybrid Trainer</p>
      <h1 className="mt-2 font-display text-5xl uppercase leading-none">Treinador<br/>híbrido<br/><span className="text-primary">inteligente</span></h1>
      <p className="mt-6 max-w-xs text-sm text-muted-foreground">Musculação + corrida + esportes em um único plano semanal — adaptado à sua evolução.</p>
...
function Chips<T extends string>({ options, value, onChange }: { options: readonly T[]; value: T | undefined; onChange: (v: T) => void }) {
  return (
    <div className="flex flex-wrap gap-2">
      {options.map((o) => (
        <button key={o} type="button" onClick={() => onChange(o)}
          className={`rounded-lg border px-3 py-2 text-sm transition ${value === o ? "border-primary bg-primary/10 text-primary" : "border-border bg-surface text-foreground"}`}>{o}</button>
      ))}
    </div>
  );
}

function WeekdayPicker({ value, onChange }: { value: WeekdayKey[]; onChange: (v: WeekdayKey[]) => void }) {
...
function Basics({ p, setP }: { p: Draft; setP: (v: Draft) => void }) {
  return (
    <div className="space-y-4">
      <H title="Quem é você?" />
      <Field label="Nome"><input className={inputCls()} value={p.name ?? ""} onChange={(e) => setP({ ...p, name: e.target.value })} placeholder="Como te chamam" /></Field>
      <div className="grid grid-cols-2 gap-3">
        <Field label="Idade"><input type="number" inputMode="numeric" className={inputCls()} value={p.age ?? ""} onChange={(e) => setP({ ...p, age: +e.target.value })} placeholder="28" /></Field>
        <Field label="Sexo"><Chips options={["M","F","Outro"] as const} value={p.sex} onChange={(v) => setP({ ...p, sex: v })} /></Field>
      </div>
    </div>
  );
}
...
function Body({ p, setP }: { p: Draft; setP: (v: Draft) => void }) {
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        <Field label="Altura (cm)"><input type="number" className={inputCls()} value={p.height ?? ""} onChange={(e) => setP({ ...p, height: +e.target.value })} placeholder="178" /></Field>
        <Field label="Peso (kg)"><input type="number" className={inputCls()} value={p.weight ?? ""} onChange={(e) => setP({ ...p, weight: +e.target.value })} placeholder="78" /></Field>
      </div>
      <Field label="% de gordura (opcional)"><input type="number" className={inputCls()} value={p.bodyFat ?? ""} onChange={(e) => setP({ ...p, bodyFat: +e.target.value || undefined })} placeholder="18" /></Field>
    </div>
  );
}

function Goal({ p, setP }: { p: Draft; setP: (v: Draft) => void }) {
  const goals: Profile["goal"][] = ["Emagrecimento","Hipertrofia","Ganho de Força","Condicionamento","Saúde Geral","Performance Esportiva","Performance Híbrida","Futebol"];
  return (
    <div>
      <H title="Seu objetivo" sub="A IA vai estruturar tudo em volta dele." />
      <div className="grid grid-cols-2 gap-2">
        {goals.map((g) => (
          <button key={g} type="button" onClick={() => setP({ ...p, goal: g })}
            className={`rounded-xl border p-4 text-left transition ${p.goal === g ? "border-primary bg-primary/10" : "border-border bg-surface"}`}>
            <p className={`font-display text-base uppercase ${p.goal === g ? "text-primary" : ""}`}>{g}</p>
          </button>
        ))}
      </div>
    </div>
...
function Level({ p, setP }: { p: Draft; setP: (v: Draft) => void }) {
  ...
  return (
    <div className="space-y-6">
      <H title="Seu nível" />
      <Field label="Musculação"><Chips options={lvls} value={p.strengthLevel} onChange={(v) => setP({ ...p, strengthLevel: v })} /></Field>
      <Field label="Corrida"><Chips options={lvls} value={p.runLevel} onChange={(v) => setP({ ...p, runLevel: v })} /></Field>
    </div>
  );
}
  ...
  return (
    <div className="space-y-6">
      ...
      <p className="font-mono text-[10px] uppercase text-muted-foreground">
        {p.gymDays.length === 0 ? "Selecione ao menos 1 dia" : `${p.gymDays.length} dia${p.gymDays.length > 1 ? "s" : ""} selecionado${p.gymDays.length > 1 ? "s" : ""}`}
      </p>
    </div>
  );
}
  ...
  return (
    <div className="space-y-6">
      ...
      <Field label={`Tempo por treino: ${p.timePerDay} min`}>
        <input type="range" min={20} max={120} step={5} value={p.timePerDay} onChange={(e) => setP({ ...p, timePerDay: +e.target.value })} className="w-full accent-[oklch(0.78_0.19_142)]" />
      </Field>
    </div>
  );
}
...
function Place({ p, setP }: { p: Draft; setP: (v: Draft) => void }) {
  ...
  return (
    <div className="space-y-6">
      <H title="Onde treina?" />
      <Field label="Local"><Chips options={["Academia","Casa"] as const} value={p.location} onChange={(v) => setP({ ...p, location: v })} /></Field>
          ...
          {EQUIPS.map((e) => {
            ...
            return (
              <button key={e} type="button" onClick={() => toggle(e)}
                className={`rounded-lg border px-3 py-2 text-sm transition ${active ? "border-primary bg-primary/10 text-primary" : "border-border bg-surface"}`}>{e}</button>
            );
          })}
        </div>
      </Field>
        ...
        {RESTRICTION_OPTIONS.map((r) => {
          ...
          return (
            <button key={r} type="button" onClick={() => toggle(r)}
              className={`rounded-lg border px-3 py-2 text-sm transition ${active ? "border-primary bg-primary/10 text-primary" : "border-border bg-surface"}`}>{r}</button>
          );
        })}
      </div>
    </div>
...
function Review({ p }: { p: Draft }) {
  return (
    <div className="space-y-4">
      ...
      <Row label="Academia" value={p.gymDays.map((d) => WEEKDAY_SHORT[d]).join(" · ") + ` (${p.gymDays.length}x)`} />
      <Row label="Tempo / sessão" value={`${p.timePerDay} min`} />
      <Row label="Local" value={`${p.location} • ${p.equipment?.length ?? 0} equip.`} />
      {p.sports.length > 0 ? (
        <div className="space-y-2">
          <p className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground">Esportes</p>
          {p.sports.map((s) => (
            <div key={s.name} className="rounded-xl border border-border bg-surface p-3">
              ...
              <p className="mt-1 font-mono text-[10px] uppercase text-muted-foreground">
                {s.days.length ? s.days.map((d) => WEEKDAY_SHORT[d]).join(" · ") : "sem dias definidos"}
              </p>
            </div>
          ))}
        </div>
